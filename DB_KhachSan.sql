...
--Đăng ký tài khoản
CREATE PROCEDURE sp_DangKyTaiKhoanSQL
    @HoTen NVARCHAR(100),
    @NgaySinh DATE,
    @GioiTinh NVARCHAR(10),
    @SoDienThoai NVARCHAR(15),
    ...
    @LoaiTaiKhoan NVARCHAR(10) = 'KH',  -- KH hoặc NV
    @MaTKMoi INT OUTPUT,
...
BEGIN
    ...
    BEGIN TRY
        -- Chuẩn hóa giới tính
        SET @GioiTinh = UPPER(@GioiTinh);
		IF @GioiTinh NOT IN ('NAM','NU')
		BEGIN
			SET @ErrorMessage = N'Giới tính không hợp lệ. Chỉ chấp nhận NAM hoặc NU.';
			RETURN;
		END


        -- Kiểm tra trùng tên đăng nhập trong bảng TaiKhoan
        ...
        -- Kiểm tra trùng login SQL Server
        ...
        DECLARE @SQL NVARCHAR(MAX) = N'CREATE LOGIN [' + @TenDN + '] WITH PASSWORD = N''' + @MatKhau + ''';';
        ...
        INSERT INTO TaiKhoan (TenDN, MatKhau, QuyenHan, LoaiTaiKhoan, MaKH, MaNV)
        VALUES (@TenDN, @MatKhau, 'USER', @LoaiTaiKhoan, @MaKH, @MaNV);
...
BEGIN
    ...
    -- Lấy thông tin đặt phòng và phiếu cọc
    SELECT 
        dp.MaDP,
        dp.NgayDat,
        dp.ThoiGianNhanPhong,
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            INSERT INTO HoaDon (MaDP, NgayLap, PTTT, TrangThai)
            VALUES (@MaDP, GETDATE(), N'Tiền mặt', N'Đang ở');  -- hoặc 'Chưa thanh toán' tùy bạn
        END
